...
// Variables de movimiento libre (modo humanoid y otros que necesiten moverse)
let moveKeys = { w: false, a: false, s: false, d: false };
const PLAYER_MOVE_SPEED = 5.5;
const PLAYER_BOUNDS = { x: 14, z: 14 };
...
function startGame(modeId, diff) {
    ...
    // ⚠️ initThree PRIMERO — debe existir scene antes de cualquier operación 3D
    if (!scene) initThree();
    updateRoomColor();
...
function endGame() {
    ...
    if (state.humanoid && state.humanoid.mesh) {
        ...
        if (yawObj) yawObj.position.set(0, 1.7, 0);
    }
...
// ════════════════════════════════════
// MOVIMIENTO DEL JUGADOR — modo humanoid
// WASD + colisión con límites del mapa
// ════════════════════════════════════
function updateHumanoidPlayerMove(dt) {
    if (!state.running || !yawObj) return;

    // Dirección relativa a la cámara (yaw actual)
    const yaw = yawObj.rotation.y;
    let dx = 0, dz = 0;

    if (moveKeys.w) { dx -= Math.sin(yaw); dz -= Math.cos(yaw); }
    if (moveKeys.s) { dx += Math.sin(yaw); dz += Math.cos(yaw); }
    if (moveKeys.a) { dx -= Math.cos(yaw); dz += Math.sin(yaw); }
    if (moveKeys.d) { dx += Math.cos(yaw); dz -= Math.sin(yaw); }

    const len = Math.sqrt(dx * dx + dz * dz);
    if (len > 0) {
        const spd = PLAYER_MOVE_SPEED * dt;
        const nx = yawObj.position.x + (dx / len) * spd;
        const nz = yawObj.position.z + (dz / len) * spd;
        // Límites del mapa humanoid
        yawObj.position.x = Math.max(-PLAYER_BOUNDS.x, Math.min(PLAYER_BOUNDS.x, nx));
        yawObj.position.z = Math.max(-PLAYER_BOUNDS.z, Math.min(PLAYER_BOUNDS.z, nz));
    }
}
...
function buildHumanoidMesh() {
    ...
    // ── Paleta low-poly realista ──────────────────────────────────
    const skin     = new THREE.MeshPhongMaterial({ color: 0xd4956a, shininess: 25, specular: 0x441100 });
    const shirt    = new THREE.MeshPhongMaterial({ color: 0x2a3f5f, shininess: 20 });
    const pants    = new THREE.MeshPhongMaterial({ color: 0x1e2833, shininess: 15 });
    const shoe     = new THREE.MeshPhongMaterial({ color: 0x111111, shininess: 80 });
    const hair     = new THREE.MeshPhongMaterial({ color: 0x1a0e06, shininess: 10 });
    const eyeMat   = new THREE.MeshPhongMaterial({ color: 0x1a0a00, shininess: 200, specular: 0x333333 });

    // ════ CABEZA ════
    const headGroup = new THREE.Group();
    const cranium = new THREE.Mesh(new THREE.SphereGeometry(0.118, 18, 16), skin);
    cranium.scale.set(1, 1.12, 1);
    headGroup.add(cranium);
    const jaw = new THREE.Mesh(new THREE.SphereGeometry(0.098, 14, 12), skin);
    jaw.scale.set(0.92, 0.7, 0.95);
    jaw.position.set(0, -0.08, 0.01);
    headGroup.add(jaw);
    const nose = new THREE.Mesh(new THREE.SphereGeometry(0.022, 8, 8), skin);
    nose.scale.set(1, 0.7, 1.2);
    nose.position.set(0, 0.0, 0.115);
    headGroup.add(nose);
    const eyeGeo = new THREE.SphereGeometry(0.02, 8, 8);
    const eyeL = new THREE.Mesh(eyeGeo, eyeMat);
    eyeL.position.set(-0.042, 0.025, 0.105);
    headGroup.add(eyeL);
    const eyeR = new THREE.Mesh(eyeGeo, eyeMat);
    eyeR.position.set(0.042, 0.025, 0.105);
    headGroup.add(eyeR);
    const hairCap = new THREE.Mesh(new THREE.SphereGeometry(0.122, 16, 10), hair);
    hairCap.scale.set(1, 0.6, 1);
    ...
    // ════ CUELLO ════
    const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.048, 0.054, 0.1, 10), skin);
    neck.position.y = 1.545;
    group.add(neck);

    // ════ TORSO ════
    const torsoGroup = new THREE.Group();
    const chest = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.28, 0.22), shirt);
    chest.position.y = 0.14;
    torsoGroup.add(chest);
    const abdomen = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.22, 0.20), shirt);
    abdomen.position.y = -0.11;
    torsoGroup.add(abdomen);
    const shBulgeL = new THREE.Mesh(new THREE.SphereGeometry(0.065, 10, 8), shirt);
    shBulgeL.position.set(-0.21, 0.14, 0);
    torsoGroup.add(shBulgeL);
    const shBulgeR = shBulgeL.clone();
    shBulgeR.position.set(0.21, 0.14, 0);
    torsoGroup.add(shBulgeR);
    torsoGroup.position.y = 1.21;
    group.add(torsoGroup);

    // ════ PELVIS ════
    const pelvis = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.14, 0.20), pants);
    pelvis.position.y = 0.935;
    group.add(pelvis);

    // ════ BRAZO IZQUIERDO ════
    const armLGroup = new THREE.Group();
    armLGroup.position.set(-0.245, 1.375, 0);
    const upperArmL = new THREE.Mesh(new THREE.CylinderGeometry(0.058, 0.048, 0.3, 10), shirt);
    upperArmL.position.y = -0.15;
    armLGroup.add(upperArmL);
    const elbowL = new THREE.Mesh(new THREE.SphereGeometry(0.052, 10, 8), shirt);
    elbowL.position.y = -0.30;
    armLGroup.add(elbowL);
    const foreArmLGroup = new THREE.Group();
    foreArmLGroup.position.y = -0.30;
    const foreArmL = new THREE.Mesh(new THREE.CylinderGeometry(0.043, 0.035, 0.28, 10), skin);
    foreArmL.position.y = -0.14;
    foreArmLGroup.add(foreArmL);
    const handL = new THREE.Mesh(new THREE.BoxGeometry(0.072, 0.082, 0.055), skin);
    handL.position.y = -0.30;
    foreArmLGroup.add(handL);
    armLGroup.add(foreArmLGroup);
    armLGroup.userData.foreArm = foreArmLGroup;
    group.add(armLGroup);

    // ════ BRAZO DERECHO ════
    const armRGroup = new THREE.Group();
    armRGroup.position.set(0.245, 1.375, 0);
    const upperArmR = new THREE.Mesh(new THREE.CylinderGeometry(0.058, 0.048, 0.3, 10), shirt);
    upperArmR.position.y = -0.15;
    armRGroup.add(upperArmR);
    const elbowR = new THREE.Mesh(new THREE.SphereGeometry(0.052, 10, 8), shirt);
    elbowR.position.y = -0.30;
    armRGroup.add(elbowR);
    const foreArmRGroup = new THREE.Group();
    foreArmRGroup.position.y = -0.30;
    const foreArmR = new THREE.Mesh(new THREE.CylinderGeometry(0.043, 0.035, 0.28, 10), skin);
    foreArmR.position.y = -0.14;
    foreArmRGroup.add(foreArmR);
    const handR = new THREE.Mesh(new THREE.BoxGeometry(0.072, 0.082, 0.055), skin);
    handR.position.y = -0.30;
    foreArmRGroup.add(handR);
    armRGroup.add(foreArmRGroup);
    armRGroup.userData.foreArm = foreArmRGroup;
    group.add(armRGroup);

    // ════ PIERNA IZQUIERDA ════
    const legLGroup = new THREE.Group();
    legLGroup.position.set(-0.105, 0.935, 0);
    const thighL = new THREE.Mesh(new THREE.CylinderGeometry(0.074, 0.060, 0.42, 12), pants);
    thighL.position.y = -0.21;
    legLGroup.add(thighL);
    const kneeL = new THREE.Mesh(new THREE.SphereGeometry(0.062, 10, 8), pants);
    kneeL.position.y = -0.435;
    legLGroup.add(kneeL);
    const shinLGroup = new THREE.Group();
    shinLGroup.position.y = -0.435;
    const shinL = new THREE.Mesh(new THREE.CylinderGeometry(0.052, 0.040, 0.40, 12), pants);
    shinL.position.y = -0.20;
    shinLGroup.add(shinL);
    const ankleL = new THREE.Mesh(new THREE.SphereGeometry(0.038, 8, 8), shoe);
    ankleL.position.y = -0.41;
    shinLGroup.add(ankleL);
    const footL = new THREE.Mesh(new THREE.BoxGeometry(0.095, 0.068, 0.195), shoe);
    footL.position.set(-0.008, -0.455, 0.042);
    shinLGroup.add(footL);
    legLGroup.add(shinLGroup);
    legLGroup.userData.shin = shinLGroup;
    group.add(legLGroup);

    // ════ PIERNA DERECHA ════
    const legRGroup = new THREE.Group();
    legRGroup.position.set(0.105, 0.935, 0);
    const thighR = new THREE.Mesh(new THREE.CylinderGeometry(0.074, 0.060, 0.42, 12), pants);
    thighR.position.y = -0.21;
    legRGroup.add(thighR);
    const kneeR = new THREE.Mesh(new THREE.SphereGeometry(0.062, 10, 8), pants);
    kneeR.position.y = -0.435;
    legRGroup.add(kneeR);
    const shinRGroup = new THREE.Group();
    shinRGroup.position.y = -0.435;
    const shinR = new THREE.Mesh(new THREE.CylinderGeometry(0.052, 0.040, 0.40, 12), pants);
    shinR.position.y = -0.20;
    shinRGroup.add(shinR);
    const ankleR = new THREE.Mesh(new THREE.SphereGeometry(0.038, 8, 8), shoe);
    ankleR.position.y = -0.41;
    shinRGroup.add(ankleR);
    const footR = new THREE.Mesh(new THREE.BoxGeometry(0.095, 0.068, 0.195), shoe);
    footR.position.set(0.008, -0.455, 0.042);
    shinRGroup.add(footR);
    legRGroup.add(shinRGroup);
    legRGroup.userData.shin = shinRGroup;
    group.add(legRGroup);
    ...
    shadowMesh.position.y = -1.692;
    group.add(shadowMesh);
    group.userData.shadow2D = shadowMesh;

    group.userData.parts = { headGroup, torsoGroup, pelvis, armLGroup, armRGroup, legLGroup, legRGroup };
    return group;
}

// Hitboxes separadas: cabeza y cuerpo para headshot
function buildHumanoidHitboxes(group) {
    // Hitbox cuerpo (torso + piernas)
    const bodyHitGeo = new THREE.BoxGeometry(0.44, 1.45, 0.28);
    const bodyHitMat = new THREE.MeshBasicMaterial({ visible: false });
    const bodyHitbox = new THREE.Mesh(bodyHitGeo, bodyHitMat);
    bodyHitbox.position.y = 0.78;
    bodyHitbox.userData.zone = 'body';
    group.add(bodyHitbox);

    // Hitbox cabeza (esfera, más difícil de golpear)
    const headHitGeo = new THREE.SphereGeometry(0.135, 8, 8);
    const headHitMat = new THREE.MeshBasicMaterial({ visible: false });
    const headHitbox = new THREE.Mesh(headHitGeo, headHitMat);
    headHitbox.position.y = 1.66;
...
function updateHumanoid(dt) {
    ...
    h.speedMultiplier = Math.min(3.0, 1.0 + elapsed / 22);
    const baseSpeed = 2.8 * DIFF[state.diff].speed * CFG.tgt.speedMult * h.speedMultiplier;

    // Inicializar velocidad actual si no existe
    if (h.velX === undefined) h.velX = 0;
    ...
    if (h.pauseTimer > 0) {
        ...
        if (h.pauseTimer <= 0) {
            ...
            h.targetSpeed = baseSpeed * (0.55 + Math.random() * 0.9);
        }
    }
    ...
    if (!h.microAccelActive && Math.random() < 0.004 * dt * 60 && elapsed > 12) {
        ...
        h.microAccelTimer = 0.1 + Math.random() * 0.2;
        h.targetSpeed = Math.min(baseSpeed * 2.8, (h.targetSpeed || baseSpeed) * (2.0 + Math.random()));
    }

    // ---- CAMBIO ESPONTÁNEO de dirección ----
    if (!h.isPaused && Math.random() < 0.007 * dt * 60) {
        h.dir *= -1;
        h.targetSpeed = baseSpeed * (0.5 + Math.random() * 1.1);
        // Micro-pausa al cambiar (inercia)
        h.isPaused = true;
        h.pauseTimer = 0.04 + Math.random() * 0.1;
    }

    if (!h.targetSpeed) h.targetSpeed = baseSpeed;

    // ---- ACELERACIÓN SUAVE (estilo Fortnite) ----
    // El personaje acelera/desacelera con inercia real
    const accelRate  = h.isPaused ? 18 : 12; // desacelera más rápido que acelera
    const targetVelX = h.isPaused ? 0 : h.dir * h.targetSpeed;
    h.velX += (targetVelX - h.velX) * Math.min(1, accelRate * dt);
    ...
    if (h.x > HMN_BOUNDS.right) {
        ...
        h.velX *= -0.3;  // rebote con pérdida de velocidad
        h.isPaused = true;
        h.pauseTimer = 0.05 + Math.random() * 0.15;
        h.dir = -1;
    ...
    // ---- SALTOS realistas ----
    h.jumpTimer -= dt;
    if (h.isGrounded && h.jumpTimer <= 0 && Math.random() < 0.016 * dt * 60) {
        const jumpSpeed = Math.abs(h.velX);
        // Saltos más altos cuando va rápido (como Fortnite)
        h.velY = HMN_JUMP_FORCE * (0.85 + jumpSpeed * 0.04);
        h.isGrounded = false;
        h.jumpTimer = 2.2 + Math.random() * 2.8;
    }

    // Física vertical con gravedad variable (caída más rápida que subida)
    if (!h.isGrounded) {
        const gravity = h.velY > 0 ? HMN_GRAVITY : HMN_GRAVITY * 1.3; // cae más rápido
        h.velY += gravity * dt;
        h.y += h.velY * dt;
        if (h.y <= 0) {
            h.y = 0;
            h.velY = 0;
            h.isGrounded = true;
            // Pequeño squash al aterrizar (escala momentánea)
            if (h.mesh) {
                h.mesh.scale.set(2.4, 1.9, 2.4);
                setTimeout(() => { if (h.mesh) h.mesh.scale.set(2.2, 2.2, 2.2); }, 80);
            }
        }
    }
    ...
    if (h.mesh.userData.shadow2D) {
        const shadowScale = Math.max(0.35, 1.0 - h.y * 0.12);
        const sd = h.mesh.userData.shadow2D;
        sd.scale.set(shadowScale, shadowScale, shadowScale);
        sd.position.y = -1.695 - h.mesh.position.y + (h.groundY || -1.7);
    }

    // ---- ROTACIÓN suave hacia dirección de movimiento ----
    const speed = Math.abs(h.velX);
    const leanAmt = (h.velX / Math.max(0.1, baseSpeed)) * 0.18; // lean lateral
    const targetRotY = leanAmt * 0.6;
    h.mesh.rotation.y += (targetRotY - h.mesh.rotation.y) * dt * 8;

    // ---- ANIMACIÓN REALISTA (estilo BloodStrike) ----
    const walkCycle = performance.now() / 1000;
    const inAir = !h.isGrounded;
    const moveRatio = Math.min(1, speed / Math.max(0.1, baseSpeed)); // 0=parado, 1=máx velocidad

    // Frecuencia del ciclo de pasos proporcional a la velocidad real
    const stepFreq = 2.8 + moveRatio * 3.5;
    const legSwing  = inAir ? 0.55 : Math.sin(walkCycle * stepFreq) * 0.38 * moveRatio;
    const legBend   = inAir ? -0.3 : Math.abs(Math.sin(walkCycle * stepFreq)) * 0.15 * moveRatio;
    const armSwing  = inAir ? -0.6 : Math.sin(walkCycle * stepFreq + Math.PI) * 0.32 * moveRatio;

    // Bob vertical del cuerpo (rebote al caminar)
    const bodyBobY  = inAir ? 0 : Math.abs(Math.sin(walkCycle * stepFreq * 2)) * 0.018 * moveRatio;
    // Sway lateral de cadera (muy característico de juegos modernos)
    const hipSway   = inAir ? 0 : Math.sin(walkCycle * stepFreq) * 0.04 * moveRatio;
    // Lean del torso hacia adelante al correr
    const torsoLean = inAir ? (h.velY > 0 ? -0.25 : 0.12) : moveRatio * 0.08;

    if (h.parts) {
        const p = h.parts;

        // ── PIERNAS: muslo + rodilla doblada (sub-grupo pantorrilla) ──
        // Muslo delante → pantorrilla dobla hacia atrás (IK-like)
        const thighFwdL  = legSwing;
        const thighFwdR  = -legSwing;
        const kneeBendL  = inAir ? 0.5 : Math.max(0, -Math.sin(walkCycle * stepFreq)) * 0.55 * moveRatio;
        const kneeBendR  = inAir ? 0.5 : Math.max(0, Math.sin(walkCycle * stepFreq)) * 0.55 * moveRatio;

        p.legLGroup.rotation.x = thighFwdL;
        p.legRGroup.rotation.x = thighFwdR;

        // Sub-grupo pantorrilla (siempre negativo = se dobla hacia atrás)
        if (p.legLGroup.userData.shin) p.legLGroup.userData.shin.rotation.x = kneeBendL;
        if (p.legRGroup.userData.shin) p.legRGroup.userData.shin.rotation.x = kneeBendR;

        // ── BRAZOS: ciclo opuesto a piernas ──
        const armFwdL = armSwing;
        const armFwdR = -armSwing * 0.65; // brazo derecho se mueve menos (sosteniendo arma imaginaria)
        const elbowBendL = inAir ? 0.8 : Math.abs(Math.sin(walkCycle * stepFreq + 0.5)) * 0.4 * moveRatio;
        const elbowBendR = 0.55 + moveRatio * 0.2; // derecho siempre ligeramente doblado

        p.armLGroup.rotation.x = armFwdL;
        p.armRGroup.rotation.x = armFwdR;

        // Antebrazos
        if (p.armLGroup.userData.foreArm) p.armLGroup.userData.foreArm.rotation.x = elbowBendL;
        if (p.armRGroup.userData.foreArm) p.armRGroup.userData.foreArm.rotation.x = elbowBendR;

        // ── TORSO: lean + sway ──
        p.torsoGroup.position.y = 1.21 + bodyBobY;
        p.torsoGroup.rotation.x = torsoLean;
        p.torsoGroup.rotation.z = hipSway;

        // ── CABEZA: compensa el sway, siempre mirando al frente ──
        p.headGroup.position.y = 1.66 + bodyBobY * 0.5;
        p.headGroup.rotation.z = -hipSway * 0.6;

        // ── PELVIS: sway opuesto ──
        if (p.pelvis) {
            p.pelvis.position.y = 0.935 + bodyBobY * 0.4;
            if (p.pelvis.rotation) p.pelvis.rotation.z = -hipSway * 0.5;
        }
    }
}
...
function _humanoidHurt(h, dmg) {
    if (!h.mesh) return;
    // Bot NO muere — solo recibe daño visual y se regenera
    h.hp = Math.max(10, h.hp - dmg);
    _humanoidUpdateHPBar();
    ...
    // Pequeño stagger hacia atrás (visual hit reaction)
    if (h.mesh) {
        h.velX *= -0.4; // rebote leve al recibir disparo
    }

    // Regenerar HP gradualmente (timer)
    clearTimeout(h._regenTimer);
    h._regenTimer = setTimeout(() => {
        if (state.running && state.modeId === 'humanoid' && h.hp < h.maxHp) {
            h.hp = h.maxHp;
            _humanoidUpdateHPBar();
        }
    }, 3000);
...
document.addEventListener('keydown', (e) => {
    ...
    if (key === 'a') { strafeKeys.a = true; moveKeys.a = true; }
    if (key === 'd') { strafeKeys.d = true; moveKeys.d = true; }
    if (key === 'w') moveKeys.w = true;
    if (key === 's') moveKeys.s = true;
});
document.addEventListener('keyup', (e) => {
    const key = e.key.toLowerCase();
    if (key === 'a') { strafeKeys.a = false; moveKeys.a = false; }
    if (key === 'd') { strafeKeys.d = false; moveKeys.d = false; }
    if (key === 'w') moveKeys.w = false;
    if (key === 's') moveKeys.s = false;
});
